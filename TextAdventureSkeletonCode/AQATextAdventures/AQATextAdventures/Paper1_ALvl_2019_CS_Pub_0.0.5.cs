...
namespace TextAdventuresCS
{
    class Program
    {
        ...
        private static void Examine(List<Item> items, List<Character> characters, string itemToExamine, int currentLocation, List<Place> places)
        {
            ...
            {
                Console.WriteLine(places[currentLocation-1].Description);
        ...
        private static bool IsInventoryFull(List<Item> items) 
        {
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                {
                    inventoryCount++;
                    //Console.WriteLine("inventoryCount",inventoryCount);
                }
            }
        ...
        private static void BubbleSort(List<Item> items)
        {
            ...
            while (!complete)
            {
                for (int i = 0; i < items.Count - 1; i++)
                {
                    ...
                    if (String.Compare(items[i].Name, items[i + 1].Name) == 1)
                    {
                        var tempItem = items[i];
                        var tempItem2 = items[i + 1];
                        items[i + 1] = tempItem;
                        items[i] = tempItem2;
                        swaps++;
                    }
        ...
        {
            Console.WriteLine("You can enter one of the following commands: go, get, use, examine, say, quit, read, move, open, close and playdice.");
        }
        ...
        private static void DropItem(List<Item> items, string itemToDrop, int currentLocation)
        {
            for(int i =0;i < items.Count; i++)
            {
                if(items[i].Name == itemToDrop && items[i].Location == Inventory)
                {
                    items[i].Location = currentLocation;
            ...
            while (!stopGame)
            {
                ...
                switch (Command)
                {
                    ...
                    case "playdice":
                        ...
                        break;
                    case "drop":
                        //DropItem(items, itemToDrop, characters[0].CurrentLocation); finish this
                    case "quit":
        ...
        static void Main(string[] args)
        {
            ...
            {
                string[] welcomeText = new string[3] { "WELCOME", "TO", "AQATEXTADVENTURES" };
                Say(welcomeText);
                PlayGame(characters, items, places);
            }
            else
